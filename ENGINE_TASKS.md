...
- [x] **Task 1:** `engine/replay.py` — boot old+new via git worktree + uvicorn subprocess, health-check both, replay tag-filtered scenarios with httpx (5s timeout, sequential), write `results.json`, clean shutdown. 
  - CLI: `python -m engine.replay --old v1.0 --new HEAD --tags emi,loan`
- [x] **Task 2:** `engine/compare.py` — the semantic diff per section 1 step 3, dot-path mismatches, writes verdict+diffs.
- [x] **Task 3:** `stub/` — a tiny throwaway FastAPI app (GET `/health` + one dummy POST) so replay can be smoke-tested before the real demo app exists. Deleted later.
